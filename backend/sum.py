...
import random, pathlib, httpx, uvicorn
...
You are MindBridge AI, a supportive and empathetic mental health assistant designed specifically for students.

Your purpose is to help students talk about their emotions, manage stress, and find healthy coping strategies. You provide a safe, non-judgmental space where students can express their thoughts and feelings.

Guidelines:
- Always respond with empathy, kindness, and patience.
- Validate the student's emotions before giving suggestions.
- Encourage healthy habits such as rest, talking to friends, journaling, exercise, or taking breaks.
- Provide practical tips for common student issues like exam stress, procrastination, anxiety, loneliness, burnout, and self-doubt.
- Encourage students to use MindBridge features such as daily mood check-ins, peer community support, and mental health resources.
- Never shame, judge, or dismiss a student's feelings.

Important Safety Rules:
- If a student expresses thoughts of self-harm or suicide, gently encourage them to reach out to trusted people, counselors, or local mental health helplines.
- Make it clear that you are a support tool but not a replacement for professional mental health care.
...
def startup():
    ...
    print("🧪 Test AI:  http://localhost:8000/api/test-ai")
...
def ask_groq(session_id: str, user_message: str) -> Optional[str]:
    """
    Sends the full conversation to Groq (Llama 3) and returns a smart reply.
    Remembers name, mood, and everything said earlier in the conversation.
    """
    ...
    # Keep last 20 messages (10 turns) for memory
    ...
    try:
        ...
        if r.status_code != 200:
            err = data.get("error", {})
            print(f"\n{'='*55}")
            print(f"❌ GROQ ERROR {r.status_code}: {err.get('message', data)}")
            if r.status_code == 401:
                print("   👉 Invalid key → get a new one at console.groq.com")
            print(f"{'='*55}\n")
            return None

        reply = data["choices"][0]["message"]["content"].strip()
        _chat_history[session_id].append({"role": "assistant", "content": reply})
        print(f"✅ Groq replied for {session_id[:12]}")
        return reply
...
# ══════════════════════════════════════════════════════════════════════════════
# TEST ENDPOINT → http://localhost:8000/api/test-ai
# ══════════════════════════════════════════════════════════════════════════════

@app.get("/api/test-ai")
def test_ai():
    """Open this in your browser to verify Groq is connected."""
    try:
        r = httpx.post(
            GROQ_URL,
            headers={
        ...
        return {
            ...
            "fix":    "401 → get a new free key at console.groq.com → API Keys"
        }
    except Exception as e:
...
def chat(msg: ChatMessage):
    db = get_db()
    try:
        # 1. Crisis check — always instant, never sent to AI
        if any(w in msg.message.lower() for w in CRISIS_WORDS):
            response  = CRISIS_RESPONSE
            is_crisis = True
        else:
            # 2. Ask Groq with full conversation memory
            reply = ask_groq(msg.session_id, msg.message)
            if reply:
                response  = reply
                is_crisis = False
            else:
                # 3. Fallback only if Groq fails
...
if __name__ == "__main__":
    uvicorn.run("sum:app", host="0.0.0.0", port=8000, reload=True)